...
def process_file(file, wavPath, spks, outPath, sr):
    if file.endswith(".wav"):
        file = file[:-4]
        resample_wave(os.path.join(wavPath, spks, file + ".wav"), f"{outPath}/{spks}/{file}.wav", sr)


def process_files_with_thread_pool(wavPath, spks, outPath, sr, thread_num=None):
    files = [f for f in os.listdir(os.path.join(wavPath, spks)) if f.endswith(".wav")]
...
if __name__ == "__main__":
    ...
    print(f"Resampling {wavPath} to {outPath} with sample rate {args.sr}")
